import { AppEnvError, getAppEnv, requireWebhookEmailConfig } from "@/lib/config/app-env";
...
function createEmailAdapter() {
  ...
  if (env.runtime.isProduction) {
    throw new AppEnvError(
      "EMAIL_DRIVER=console is intended for local or non-production environments. Configure EMAIL_DRIVER=webhook and EMAIL_WEBHOOK_URL before production deployment.",
      "EMAIL_DRIVER"
    );
  }
