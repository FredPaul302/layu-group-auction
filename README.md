...
- Identity verification:
  `IDENTITY_VERIFICATION_PROVIDER`, `DIDIT_API_KEY`, `DIDIT_WORKFLOW_ID`, `DIDIT_WEBHOOK_SECRET`, `DIDIT_BASE_URL`
  Use `IDENTITY_VERIFICATION_PROVIDER=didit` for the active private-beta provider. The app creates Didit hosted sessions and receives signed decisions at `/api/didit/webhook`.
- Persona:
  `PERSONA_TEMPLATE_ID`, `PERSONA_ENVIRONMENT_ID`, `PERSONA_SUBDOMAIN`, `PERSONA_WEBHOOK_SECRET`
  Persona is optional and no longer required for private-beta deployment unless `IDENTITY_VERIFICATION_PROVIDER=persona`. `PERSONA_API_KEY` is reserved for future server-to-server Persona API calls and is not required by the current hosted-flow/webhook implementation.
- Email:
...
- Set `IDENTITY_VERIFICATION_PROVIDER=didit` plus `DIDIT_API_KEY`, `DIDIT_WORKFLOW_ID`, and `DIDIT_WEBHOOK_SECRET` before accepting identity verifications
- Configure the Didit webhook URL as `https://auction.layu.inc/api/didit/webhook`; the hosted-flow callback is generated by the app when creating a session
