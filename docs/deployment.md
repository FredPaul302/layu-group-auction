...
Active private-beta provider:

- `IDENTITY_VERIFICATION_PROVIDER=didit`
- `DIDIT_API_KEY`
- `DIDIT_WORKFLOW_ID`
- `DIDIT_WEBHOOK_SECRET`
- optional `DIDIT_BASE_URL`, defaulting to `https://verification.didit.me`

Configure the Didit webhook URL in the Didit console as:

- `https://auction.layu.inc/api/didit/webhook`

The Didit hosted-flow callback is generated by the app when creating a session. Callback query parameters are non-authoritative; only the signed Didit webhook approves or rejects local verification.

### Persona

Persona remains optional. If `IDENTITY_VERIFICATION_PROVIDER=persona`:
...
- Didit webhook configured and `DIDIT_WEBHOOK_SECRET` set
...
Configure Didit to send webhooks to:

- `POST /api/didit/webhook`
- Production URL: `https://auction.layu.inc/api/didit/webhook`

Hosted Didit returns are generated with this callback:

- `GET /api/verifications/didit/callback`

The callback records only a safe returned/pending state. It does not approve, reject, or finalize verification based on query parameters. The webhook is authoritative and must be signed.

Before turning Didit on in production:

- set `IDENTITY_VERIFICATION_PROVIDER=didit`
- set `DIDIT_API_KEY`
- set `DIDIT_WORKFLOW_ID`
- set `DIDIT_WEBHOOK_SECRET`
- verify the webhook can reach the app over HTTPS
- confirm `APP_URL` uses the real public origin
...
- set `IDENTITY_VERIFICATION_PROVIDER=persona`
...
- Didit callback and webhook paths are reachable when Didit is active
- Persona callback and webhook paths are reachable if Persona is explicitly active
